...
if [[ "$RESET" == true ]]; then
  echo "[warn] Reset requested, clearing containers and local data"
  if [[ "$YES" != true ]]; then
    read -r -p "Confirm data removal? [y/N] " confirm
  ...
  rm -rf ./data/fe/doris-meta/* ./data/fe/log/* ./data/be/log/* 2>/dev/null || true
...
echo "[2/4] Ensuring local data directories exist..."
mkdir -p ./data/fe/doris-meta ./data/fe/log ./data/be/log
...
echo "[4/4] Waiting for Doris FE/BE and registering canonical backend..."
...
echo "[4.5/4] Starting API and frontend after Doris is stable..."
...
echo "  - Doris UI:  http://localhost:38030"
echo "  - API:       http://localhost:38018"
echo "  - Health:    http://localhost:38018/api/health"
echo "  - MySQL:     mysql -h127.0.0.1 -P39030 -uroot"
